package Test::RedisServer::Summoner;
...
sub new {
  ...
  $self->{auto_start} = exists $params{auto_start} ? $params{auto_start} : 1;
  $self->{pid}        = $params{pid};
  ...
  if ($self->{conf}{loglevel} and $self->{conf}{loglevel} eq 'warning') {
      warn "Test::RedisServer::Summoner does not support \"loglevel warning\","
...
Test::RedisServer::Summoner - redis-server runner for tests.
...
=head1 SYNOPSIS
    ...
    my $redis_server;
    eval {
        $redis_server = Test::RedisServer::Summoner->new;
...
=head2 new(%options)
    ...
    my $redis_server = Test::RedisServer::Summoner->new(%options);
...
Your conf parameter will be:
    ...
    Test::RedisServer::Summoner->new( conf => {
        port      => 9999,
        databases => 16,
        save      => '900 1',
    });
...
for you, but you start Test::RedisServer::Summoner via exec (e.g. with Test::TCP), you
...
with L<Test::TCP>, L<Proclet> or etc.
    ...
    test_tcp(
        ...
        server => sub {
            my ($port) = @_;
            my $redis = Test::RedisServer::Summoner->new(
                auto_start => 0,
                conf       => { port => $port },
                tmpdir     => $tmp_dir,
            );
...
This parameter is designed to pass directly to L<Redis> module.
    ...
    my $redis_server = Test::RedisServer::Summoner->new;
    my $redis = Redis->new( $redis_server->connect_info );
